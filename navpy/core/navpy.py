...
def rot(angle, axis, input_unit = 'rad', output_type = 'ndarray'):
    """
    This function returns the DCMs corresponding to a rotation around the 
    specified axis by the specified angles
    
    Parameters
    ----------
    angle : {(N,), (N,1), or (1,N)}
            A sequence of angles to rotate by
    axis : 0, 1, 2, 'X', 'Y' or 'Z'
    ...
    angle, N = _input_check_Nx1(angle)

    if N > 1 and output_type != 'ndarray':
...
def angle2dcm(*argv, input_unit='rad',
    ...
    if len(rotation_sequence) != 3:
        raise ValueError('Sequence must be 3 letters')

    if input_unit == 'deg':
        rotAngles = np.deg2rad(rotAngles)
    ...
    try:
        ...
        C = np.einsum('nij, njk, nkm -> nim', R[0,:,:,:], R[1,:,:], R[2,:,:])
    except AttributeError:
        # Older NumPy without einsum
        C = np.zeros((N, 3, 3))
        for i, (R0, R1, R2) in enumerate(zip(R[0], R[1], R[2])):
            C[i] = R0.dot(R1.dot(R2))
